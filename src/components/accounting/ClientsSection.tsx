...
export function ClientsSection({ branchId }: ClientsSectionProps = {}) {
  ...
  // Only إشعار دائن is surfaced here — credit_note rows with positive
  // amount, meaning the office owes the customer wallet credit.
  // (Negative-amount credit_notes representing إشعار مدين live behind
  // a future feature flag and are intentionally hidden from this view.)
  ...
  // credit issued). Clamped at zero — when negative, the office owes
  // the customer; that direction is shown by the "الخارج للعملاء"
  // pill instead.
  const totalOutstanding = Math.max(
    0,
    totalBilled - totalReceived - totalDisbursed - totalCreditNotes,
  );
  const totalOutflow = totalDisbursed + totalCreditNotes;

  return (
    <div className="space-y-4">
      {/* Summary pills — four-card row mirrors the شركات tab layout
...
// ──────────────────────────────────────────────────────────────
// Generic receipts table — handles all 4 receipt-flavored tabs
// ──────────────────────────────────────────────────────────────
