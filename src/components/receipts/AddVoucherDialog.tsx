...
// ─── CompanyPicker ─────────────────────────────────────────────
//
// Sibling of ClientPicker / BrokerPicker — search-as-you-type by
// company name. Filter rule: only companies that have at least one
// non-ELZAMI policy are surfaced (pure إلزامي companies don't have
// an open accounting relationship with the office — customer pays
// them directly via visa_external).
//
// No balance breakdown is rendered in this list on purpose: the
// agent confirmed the picker should feel like the customer picker,
// and the full running-account view appears in the next dialog
// (AddSettlementDialog / AddCompanyCreditNoteDialog) after the
// company is chosen. Keeping the list minimal also makes scanning
// fast when an agent has dozens of companies.
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          {value ? (
            <span className="font-semibold truncate">{displayName(value)}</span>
          ) : (
            <span className="text-muted-foreground inline-flex items-center gap-2">
              <Search className="h-3.5 w-3.5" />
              ابحث عن شركة...
