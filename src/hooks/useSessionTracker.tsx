...
export function useSessionTracker() {
  ...
  useEffect(() => {
    ...
    // (Removed: per-row Realtime subscription on user_sessions. The
    // table was dropped from the supabase_realtime publication for
    // privacy — the row carries IP / user-agent / device columns and
    // an external scan flagged the broadcast surface. The 30-second
    // heartbeat below already polls kicked_at as the documented
    // fallback, so kicks still fire — just up to 30s later.)
    ...
    // sign the user out on the next tick (≤30s lag). With user_sessions
    // off the supabase_realtime publication this is the only kick
    // mechanism — instant kicks via postgres_changes were retired.
