-- Renewals reports previously auto-detected "renewed" by date-matching a
-- newer policy of the same type/car for the customer. Business rule change:
-- a customer is "تم التجديد" *only* when an agent clicks تجديد in the
-- Smart Assistant (مساعد التجديد), which writes to `renewal_followups`
-- with status = 'renewed'. Both reports below now key off that table:
--   - report_renewals (التجديدات tab) excludes clients whose followup
--     for the policy's expiry month is 'renewed'.
--   - report_renewed_clients (تم التجديد tab) includes only clients whose
--     followup for the selected month is 'renewed'.
-- "التجديد بواسطة" now reflects who clicked the button
-- (renewal_followups.updated_by), not who created the new policy.
-- New-policy display columns (price, start date, count) remain via a
-- LEFT JOIN — they will be NULL until the new policy is actually entered.
...
BEGIN
  ...
  RETURN QUERY
  WITH renewed_clicks AS (
    SELECT DISTINCT ON (rf.client_id)
      rf.client_id, rf.updated_by AS renewed_by
    FROM renewal_followups rf
    WHERE rf.follow_up_month = v_month_str
      AND rf.status = 'renewed'
      AND (v_is_sa OR rf.agent_id = v_agent_id)
    ORDER BY rf.client_id, rf.updated_at DESC NULLS LAST
  ),
  expiring_policies AS (
    SELECT p.id, p.client_id, p.car_id,
           p.policy_type_parent AS ptype, p.policy_type_child AS pchild,
    ...
    WHERE p.end_date BETWEEN v_month_start AND v_month_end
      ...
      AND EXISTS (
        SELECT 1 FROM renewed_clicks rc WHERE rc.client_id = p.client_id
      )
  ),
  renewal_mappings AS (
    SELECT DISTINCT ON (ep.id)
      ep.id AS old_policy_id, ep.client_id, np.id AS new_policy_id,
      np.policy_type_parent AS new_ptype, np.policy_type_child AS new_pchild,
  ...
  client_aggregates AS (
    SELECT ep.client_id, c.full_name AS client_name,
      ...
      ARRAY_AGG(DISTINCT
        CASE WHEN ep.ptype::text = 'THIRD_FULL' AND ep.pchild IS NOT NULL
             THEN ep.pchild::text
             ELSE ep.ptype::text END
      ) AS policy_types,
      ...
      ARRAY_AGG(DISTINCT
        CASE WHEN rm.new_ptype::text = 'THIRD_FULL' AND rm.new_pchild IS NOT NULL
             THEN rm.new_pchild::text
             ELSE rm.new_ptype::text END
      ) FILTER (WHERE rm.new_ptype IS NOT NULL) AS new_policy_types,
      ...
      rc.renewed_by AS renewed_by_admin_id
    FROM expiring_policies ep
    JOIN clients c ON c.id = ep.client_id
    JOIN renewed_clicks rc ON rc.client_id = ep.client_id
    LEFT JOIN renewal_mappings rm ON rm.old_policy_id = ep.id
    WHERE (v_is_sa OR c.agent_id = v_agent_id)
    GROUP BY ep.client_id, c.full_name, c.file_number, c.phone_number, rc.renewed_by
  )
