...
--   * Income  = SUM(policies.profit) for non-ELZAMI policies in period
--                (column is already net per policy)
--   * Expense = SUM(expenses.amount) where voucher_type='payment'
--                (agent-entered payouts on the accounting page)
--   * Net profit KPI uses the same pair so the dashboard is internally
--     consistent.
--   * Top-companies ranks by agent profit, not gross insurance_price.
--   * Period-scoped totals exposed via dashboard_income_expense_totals
--     so the chart header tracks the selected period pill while the
--     6-month trend series remains a steady monthly chart below it.
  ...
  SELECT
    ...
    (SELECT COUNT(DISTINCT p.car_id) FROM public.policies p
      WHERE p.cancelled = false AND p.deleted_at IS NULL
        ...
        AND (v_is_sa OR p.agent_id = v_agent_id))::bigint AS cars_insured,
...
-- Top companies by agent PROFIT (not gross insurance_price). Packages
-- still count as one transaction. Excludes ELZAMI since it contributes
-- zero profit to the agent.
...
-- Period-scoped variants of the donut + debt buckets, so the period
-- pill really does filter everything on the dashboard. Donut uses
-- created_at (when the policy was issued, matching the KPI window);
-- debt buckets use start_date (the policy's active window).
