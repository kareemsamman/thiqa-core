-- Debt tracking: include ELZAMI policies so packages can surface their
-- office_commission (the agent's earning) alongside the sibling non-ELZAMI
-- policies in a single aggregated row on the client. The HAVING clause still
-- hides fully-paid policies.
