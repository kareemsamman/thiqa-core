-- Renewals & renewed-clients reports were emitting `THIRD_FULL` for every
-- comprehensive/third-party policy because they aggregate only the parent
-- enum. The label maps `THIRD_FULL` → "طرف ثالث / شامل" (a placeholder for
-- "either"), even though the policy itself is one or the other. Resolve to
-- the child enum (`THIRD` / `FULL`) when present, mirroring what
-- `report_created_policies` already does.
