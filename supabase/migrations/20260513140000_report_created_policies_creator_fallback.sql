-- ============================================================
-- report_created_policies → fall back to email when the
-- creator's profile.full_name is null.
--
-- Several profiles in the wild were created without a
-- full_name set, so the "أنشأ" column rendered "-" even when
-- the row had a perfectly valid created_by_admin_id. Coalesce
-- to split_part(pr.email, '@', 1) so the local-part of the
-- email shows in that case (a.h.musaffer instead of nothing).
-- ============================================================
...
BEGIN
  ...
  WITH grouped_policies AS (
    SELECT
      ...
      (ARRAY_AGG(COALESCE(NULLIF(TRIM(pr.full_name), ''), split_part(pr.email, '@', 1))))[1] as grp_created_by_name,
