...
-- Update the calculate_policy_company_payment function to handle empty rules = full profit
...
BEGIN
  ...
  IF p_policy_type_parent = 'THIRD_FULL' THEN
    ...
    IF p_policy_type_child = 'FULL' THEN
      -- Get discount (fixed ₪ amount that REPLACES third component)
      ...
      -- If no full rules exist (no percent, no discount, no min, no third), full profit
      IF v_full_percent = 0 AND v_discount = 0 AND v_min_price = 0 AND v_third_price = 0 THEN
        RETURN QUERY SELECT 0::numeric, p_insurance_price;
        RETURN;
      END IF;

      -- DISCOUNT is a fixed amount that replaces third_price, NOT a percentage
  ...
  -- Default fallback: No rules found = full profit (empty = full profit rule)
  RETURN QUERY SELECT 0::numeric, p_insurance_price;
END;
$function$;
