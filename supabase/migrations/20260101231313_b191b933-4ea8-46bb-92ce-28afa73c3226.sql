-- Update report_renewals to include created_by_name and accept created_by filter
DROP FUNCTION IF EXISTS public.report_renewals(date, integer, text, text, integer, integer);
...
-- Add a service-level function for edge functions to use (no auth check)
...
-- Add columns for automatic reminder tracking
ALTER TABLE public.sms_settings
ADD COLUMN IF NOT EXISTS renewal_reminder_1month_enabled boolean DEFAULT true,
ADD COLUMN IF NOT EXISTS renewal_reminder_1week_enabled boolean DEFAULT true;